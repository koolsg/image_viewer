"""Simple schema migration framework for Thumbnail DB.

This module defines a small migration registry and helpers for applying
upgrades and downgrades for the thumbnails DB. The ThumbDB class will call
these helpers when it detects a user_version < latest_version.

This migration set is intentionally small: v1 adds `thumb_width`,
`thumb_height`, and `created_at` columns. The downgrade path recreates the
legacy table without those columns.
