...
class DisplayController:
    ...
    def open_folder(self) -> None:
        ...
        # Set folder in shared model (triggers directoryLoaded signal)
        viewer.fs_model.setRootPath(dir_path)

        # Get initial file list from model
        files = viewer.fs_model.get_image_files()
        viewer.image_files = files  # Sync with model
        ...
        viewer.setWindowTitle(
            f"Image Viewer - {os.path.basename(files[viewer.current_index])}"
        )
        self.display_image()
        ...
        try:
            ...
            pass
        _logger.debug(
            "folder opened: %s, images=%d", dir_path, len(files)
        )
